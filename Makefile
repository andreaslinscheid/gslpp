...
SOURCE_FILES := $(shell find -L ./gslpp/ -type f | grep .*\.cpp | grep -vP "(?:\./)?tests?\/*" )
...
BUILDLIBLIST := $(shell find -L ./gslpp/ -type f | grep .*\.cpp | grep -vP "(?:\./)?tests?\/*"|sed -e 's/\.\/gslpp\/\(\w*\).*/\1/'|uniq)
...
clean:
	@rm -rf \
	deps.debug objs.debug \
	deps.release objs.release \
	lib debuglib
